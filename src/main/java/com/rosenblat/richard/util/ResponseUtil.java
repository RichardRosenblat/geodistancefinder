...
public final class ResponseUtil {
    ...
    public static void checkResultsSize(GeocodeResponse geocoderesponse) {
        if (geocoderesponse.getResults().size() == 0) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Error while getting request");
        }
    }
